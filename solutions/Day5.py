...
def part2(seat_codes):
    seatids = set([calculate_seatid(decode(row_code),decode(col_code)) for row_code,col_code in seat_codes])
...
def process_input(file):
    def process_code(code):
        code=code.replace('F','0').replace('L','0').replace('B','1').replace('R','1')
